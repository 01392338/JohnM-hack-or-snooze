$(async function () {
	...
	const $navSubmit = $('#nav-storySubmit');
   ...
   */
	...
	/**
 * Event handler for Navigation to Story Submit form
 */

	$('body').on('click', '#nav-storySubmit', async function () {
		$submitForm.toggle();
	});
   ...
   */
	...
	/**
 * Event listener for user submitting a story.
 *  If successful then what DEBUG?
 */
	...
	/**
 * Event listener for clicking favorites
 *  If successful then what DEBUG?
 */

	$navFavs.on('click', async function (evt) {
		hideElements();
		if (currentUser) {
			generateFaves();
   ...
   */
	...
	console.log(await axios.get(`${BASE_URL}/stories`));
});
